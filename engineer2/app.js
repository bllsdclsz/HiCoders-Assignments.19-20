...
for(let i = 0;i<dikdortgenOlanlar.length; i = i +2){
    ...
    {let dikdort = calculationMethods.rectangle_area(dikdortgenOlanlar[i], dikdortgenOlanlar[i + 1]);
...
for(let i = 0;i<daireOlanlar.length;i++){
    if(daireOlanlar[i]>0){let dairarea = calculationMethods.circle_area(daireOlanlar[i]);
...
for(let i = 0;i<kupHacimleri.length;i++){
    let kuphac = calculationMethods.cube_volume(kupHacimleri[i]);
